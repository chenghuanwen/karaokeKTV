...
public class DanmakuView extends View {
    ...
    private int mMaxRow = 5; //�?��几条弹道
    private int mPickItemInterval = 1;//每隔多长时间取出�?��弹幕来播�?
    private int mMaxRunningPerRow = 5; //每条弹道上最多同时有几个弹幕在屏幕上运行
    private float mStartYOffset = 0.1f; //第一个弹道在Y轴上的偏移占整个View的百分比
    private float mEndYOffset = 0.9f;//�?���?��弹道在Y轴上的偏移占整个View的百分比
    ...
    private boolean mShowDebug = false;//显示弹道，ps�?
...
//                                Log.d(TAG, "Not find vacant channel, add it back");
    ...
    /**随机寻找�?��可以播放弹幕而不会发生碰撞的弹道,返回弹道的Y坐标在mChannelY上的index,如果没有找到则返�?1*/
    private int findVacant(IDanmakuItem item) {
        try {//fix NPT exception
            ...
            for (int i = 0; i < mMaxRow; i++) {
                ...
                if (list.size() > mMaxRunningPerRow) {//每个弹道�?��mMaxRunning个弹�?
                    continue;
                }
